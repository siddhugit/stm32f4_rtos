...
{
	// Don't call printf() from ISR
	//printf("User button is pressed ISR - 1\r\n");
	// User button
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
	if(GPIO_Pin == GPIO_PIN_0)
	{
		++button_press_count;
		xSemaphoreGiveFromISR(xWorkSemaphore, &xHigherPriorityTaskWoken);
	}
}
...
void user_button_task( void * args )
{
    for( ;; )
    {
		if (xSemaphoreTake(xWorkSemaphore,portMAX_DELAY))
		{
			printf("User button is pressed - %lu times\r\n",button_press_count);
		}
    }
...
int main(void)
{
  ...
  xWorkSemaphore = xSemaphoreCreateCounting(10,0);
